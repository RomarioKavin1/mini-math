...
async function fusionPlusSubmitOrder(
  ...
  if (!responseText || responseText.trim() === '') {
    return {
      orderHash: (body.order as any).salt,
      success: true,
  ...
  protected async _nodeExecutionLogic(): Promise<OutputType[]> {
    ...
    const globalState =
      this.workflowGlobalState.getGlobalState<{
        wallet?: { network?: string; address?: string; ownerAddress?: string }
      }>() ?? {}
    ...
    let chain = (rawConfig.chain || walletInfo?.network || 'ethereum') as AppNetworkName
    ...
    const resolveToken = async (
      ...
      if (typeof tokenInput === 'string' && tokenInput.length > 0) {
        ...
        try {
          ...
          const tokenList = (await response.json()) as Record<string, any>
    ...
    const upstreamAccountName =
      (walletInfo as any)?.accountName && typeof (walletInfo as any).accountName === 'string'
        ? ((walletInfo as any).accountName as string)
        : null
    ...
    const quoteId = (quote as any).quoteId as string | undefined
    const quoteResponse = quote as any
    ...
    const built = await fusionPlusBuildEvmOrder(quoteId, buildBody, fusionConfig, false)

    const typedData = (built as any).typedData as
      | {
      ...
      | undefined
    const extension = ((built as any).extension as string | undefined) ?? '0x'
    const orderHash = (built as any).orderHash as string | undefined

    if (!typedData) {
      this.logger.error('Fusion Plus build response structure', {
        fullResponse: JSON.stringify(built),
        responseKeys: Object.keys(built as any),
      })
      throw new Error(
        `Fusion Plus build did not return typedData. Response keys: ${Object.keys(built as any).join(', ')}`,
      )
    }
    ...
    this.logger.info('Signing typed data with CDP', {
      ...
      domainName: (typedData.domain as any)?.name,
      domainChainId: (typedData.domain as any)?.chainId,
      verifyingContract: (typedData.domain as any)?.verifyingContract,
    })
    ...
    const sigRaw = signResult.signature as any
    ...
    if (typeof sigRaw === 'string') {
      ...
      if ('r' in sigRaw && 's' in sigRaw && 'v' in sigRaw) {
        ...
        let r = String(sigRaw.r).replace(/^0x/i, '').padStart(64, '0')
        let s = String(sigRaw.s).replace(/^0x/i, '').padStart(64, '0')
    ...
    if ('srcChainId' in cleanOrder) {
      delete (cleanOrder as any).srcChainId
    }
    ...
    this.logger.info('Fusion Plus submit response', {
      submitResultKeys: Object.keys(submitResult as any),
      submitResult: submitResult as any,
      buildOrderHash: orderHash,
    })

    const submittedOrderHash =
      (submitResult as any)?.orderHash && typeof (submitResult as any).orderHash === 'string'
        ? ((submitResult as any).orderHash as string)
        : undefined
      ...
      if (readyFills && readyFills.fills && readyFills.fills.length > 0) {
        ...
        for (const secret of secrets) {
          try {
            ...
            this.logger.error('Failed to submit secret', { error })
          }
        }
