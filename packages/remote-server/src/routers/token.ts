...
const logger = makeLogger('token-router')
...
export function create(): Router {
  ...
  router.get('/tokens/:chainId', async (req: Request, res: Response) => {
    try {
      ...
      // Transform the token list to match old backend format
      // 1inch returns an object keyed by lowercased address
      const transformedTokens = Object.entries(tokenList as Record<string, any>).map(
        ([address, tokenData]: [string, any]) => ({
          address,
